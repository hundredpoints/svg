...
import RNSvg, { SvgProps } from 'react-native-svg';
...
export function Svg({ style, ...props }: SvgProps): JSX.Element {
  const tabIndex = ({ tabIndex: '-1' } as unknown) as SvgProps;
  return (
    <RNSvg
      focusable={false}
      {...tabIndex}
      {...props}
      style={[style, { outline: 'none' } as ViewStyle]}
    />
  );
}
